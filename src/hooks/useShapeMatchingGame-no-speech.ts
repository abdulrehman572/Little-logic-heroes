...
const useShapeMatchingGame = () => {
  ...
  const checkCollision = (shapeX: number, shapeY: number, target: Target): boolean => {
    const targetCenterX = target.x + 50; // Target is 100px wide
    const targetCenterY = target.y + 50; // Target is 100px tall
    const distance = Math.sqrt(
      Math.pow(shapeX - targetCenterX, 2) + Math.pow(shapeY - targetCenterY, 2)
    );
    return distance < 60; // 60px collision radius
  };
  ...
  // Handle shape drop - returns true if correct, false if incorrect
  const handleShapeDrop = useCallback(async (shapeId: string, x: number, y: number): Promise<boolean> => {
  ...
  const nextLevel = useCallback(() => {
    ...
    // Move shapes back to starting position
